...
def clean_function(code):
    ...
    code = re.sub(r'/\*\s*(FLAW|FIX|good|bad|incl)\s*\*/', '', code, flags=re.IGNORECASE)
    # Collapse excess blank lines
    code = re.sub(r'\n{3,}', '\n\n', code)
    return code.strip()

# Load your parsed output
df = pd.read_json("data/classification_processed/juliet_functions.json", lines=True)
...
df.to_json("data/classification_processed/juliet_cleaned.json", orient='records', lines=True)
print("Saved to data/classification_processed/juliet_cleaned.json")
